...
class Solver(object):
    ...
    def vali(self, vali_loader):
        ...
        for i, (input_data, _) in enumerate(vali_loader):
            ...
            for u in range(len(prior)):
                series_loss += (torch.mean(my_mse_loss(series[u], (
                ...
                prior_loss += (torch.mean(
                    my_mse_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                       self.win_size)),
                               series[u].detach())) + torch.mean(
                    my_mse_loss(series[u].detach(),
                               (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                       self.win_size)))))
                ...
                for u in range(len(prior)):
                    series_loss += (torch.mean(my_mse_loss(series[u], (
                        ...
                        my_mse_loss((prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                           self.win_size)).detach(),
                                   series[u])))
                    prior_loss += (torch.mean(my_mse_loss(
                        (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                self.win_size)),
                        series[u].detach())) + torch.mean(
                        my_mse_loss(series[u].detach(), (
    ...
    def test(self):
        ...
        for i, (input_data, labels) in enumerate(self.train_loader):
            ...
            for u in range(len(prior)):
                if u == 0:
                    series_loss = my_mse_loss(series[u], (
                            prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                   self.win_size)).detach()) * temperature
                    prior_loss = my_mse_loss(
                        (prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                self.win_size)),
